print("*** LOADING flightdirector.nas ... ***");
######################################################################
#                                                                    #
#                 FLIGHT DIRECTOR/AUTOPILOT SETTINGS                 #
#                                                                    #
######################################################################
# Syd Adams
...
var set_nav_mode = func() {
  ...
  if (NAVSRC == "NAV1") {
    if (getprop("/instrumentation/nav[0]/data-is-valid")) {
      if(getprop("/instrumentation/nav[0]/nav-loc")) {
...
var update_nav = func() {
  ...
  if (NAVSRC == "NAV1") {
    ...
    if (getprop("/instrumentation/nav[0]/has-gs")) {
      sgnl = "ILS1";
    }
    if (sgnl == "ILS1") {
      gs = 1;
    ...
    if ((getprop("/instrumentation/gps/mode") == "obs") and (getprop("/instrumentation/nav[0]/slaved-to-gps") == 1)) {
...
var update_fd = func() {
  ...
  stick_pos = (elev_ctrl > deadZ_pitch or -deadZ_pitch > elev_ctrl
